...
sess = tf.Session(graph=tf.get_default_graph(), config=session_conf)         #Without these two lines, the code will throw environment error due to session mismatch
K.set_session(sess)

# In rare cases, nan value occurs due to the weight projection.... If that happens, please rerun the code...
# Running for around 60 epochs with a batch size of 512-1024 gives good results.. 
...
class AdversarialFairModel(object):
    ...
    def train_dem_parity(
        ...
        for epoch in range(epochs):
            outer1 = tqdm(total=num, desc='Training Adversary', position=0)
            ...
            for b in range(0, len(X), batch_size):
                ...
                c = c + 1
                l2 = loss_a([X[b:b+batch_size], z[b:b+batch_size]])
                adv = gradients_adv([X[b:b+batch_size], z[b:b+batch_size]])

                for i in range(2):
    
                    sess.run(tf.assign_sub(adversary.trainable_weights[i+8], learning_rate*adv[i]))               #adversary weight update
                loss_adv += l2 
                
                del l2,adv
            
            outer2 = tqdm(total=num, desc='Training Classifier', position=0)
            for b in range(0, len(X), batch_size):
                outer2.update(1)

                l1 = loss_p([X[b:b+batch_size], y[b:b+batch_size]])
                clas = gradients_class([X[b:b+batch_size], y[b:b+batch_size]])
                clasadv = gradients_class_adv([X[b:b+batch_size], z[b:b+batch_size]])  
                
                for i in range(len(adversary.trainable_weights[:-2])):
            ...
            print('Adversarial Accuracy: ',acc2)
            del y_pred,y_pred1
